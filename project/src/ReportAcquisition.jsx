  ...
  return (
    <section className="jt-report-result__section">
      ...
      <div style={ACQ_ORD_BOX}>
        ...
        {/* ★ 260921 신설 — 서울·부산·인천 조례는 제78조 «제8항»을, 다른 시·도는 «제6항»을
            인용한다(조례를 고친 시점이 서로 다르기 때문). 어느 쪽이 맞는지는 이 화면이
            판정하지 않는다 — 조문을 고친 시점과 현행 법률의 항 번호가 다를 수 있다는
            사실만 고정으로 알린다. */}
        <p style={{ margin: '0 0 10px', fontSize: 13, opacity: 0.85 }}>⚠️ 조례가 인용한 법 조항 번호는 조례를 고친 시점의 것이라, 현행 법률의 항 번호와 다를 수 있습니다.</p>
    ...
    try {
      ...
      try {
        ...
        if (commentary) { /* 신규 유형 — 위에서 이미 고정 문구를 넣었다. 덮어쓰지 않는다 */ }
          ...
          saving_ideas: [
            /* 260921: 종전 제목은 「생애최초·신혼부부 감면」이었다 — 신혼부부 경감(지특법 §36의2①)은 조문에
               「2020년 12월 31일까지」로 적혀 있고 연장되지 않았다(law.go.kr API 260921 조회, 엔진도 같은 날 0 으로 정정).
               끝난 제도를 «절세 여지»로 안내하지 않는다. */
            { title: '생애최초 주택 구입 감면', detail: '요건을 충족하면 취득세 감면(최대 200만)을 받을 수 있으니 자격을 확인하세요(지방세특례제한법 §36의3).' },
