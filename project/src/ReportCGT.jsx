...
const CGT_QS = [
  {
    ...
    opts: [
      ...
      ['presale', '분양권', '아파트 등 분양권 · 60~70% 단일세율'],
      ['occupancy_orig', '입주권 (원조합원)', '재개발·재건축 직접 조합원 · 장특공제 적용'],
      ['occupancy_succ', '입주권 (승계취득)', '조합원에게서 산 입주권 · 장특공제 배제(§95②)'],
      ['replacement', '대체주택 (재개발 중 거주용)', '내 집 재개발 동안 살려고 산 다른 집을 파는 경우 · §156의2⑤ 비과세'],
      ['commercial', '상가·오피스텔·토지 (비주택)', '기본세율·표1 장특 — 비과세 없음'],
    ],
  },
  {
    id: 'nonHouseType',
  ...
  // ───────── 양도 시 정보 ─────────
  {
    id: 'transferDate',
    section: '양도 시 정보',
    q: '언제 양도하나요? (매도 잔금·등기일 · 오늘로 채워둠)',
    sub: '⚠️ 양도일이 세액을 크게 좌우합니다 — 다주택 중과 한시 배제(유예) 종료(복원) 시점 전후로 같은 2주택도 세금이 크게 달라집니다. 본 계산은 2026.5.10 복원 기준이며, 정확한 적용은 양도 시점 법령으로 확인하세요. 실제 매도 예정일로 바꿔 비교해 보세요. (예: 20260815)',
    date: true,
    optional: true,
    allowFuture: true,
  },
  ...
  // ───────── 주택 상황 ─────────
  {
    ...
    sub: '다주택자 중과세 판정의 핵심 요건입니다. 2025.10.20 시행으로 현재 서울 전역(25개 자치구)과 경기 12곳(과천·광명·성남·수원·안양 동안·용인 수지·의왕·하남 등)이 조정대상지역입니다. (수시 변경되므로 양도 시점 국토교통부 고시로 확인하세요.)',
  ...
  // ───────── 입주권 상황 (조합원입주권 비과세 사실관계) ─────────
  {
    ...
    showIf: (a) => a.assetType === 'occupancy_orig' || a.assetType === 'occupancy_succ',
    opts: [
  ...
  },
  {
    ...
    showIf: (a) => (a.assetType === 'occupancy_orig' || a.assetType === 'occupancy_succ') && !!a.ipjuMgmtDate,
  },
  // ───────── 대체주택 상황 (시행령 §156의2⑤) ─────────
  {
    id: 'replApprovalDate',
