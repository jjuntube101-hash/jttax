...
function JTReportComprehensive({ setRoute, onBack }) {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      if (compFallbackGaps(answers, { precise: true }).length > 0) {
      ...
      return (
        <div className="jt-container">
          <JTReportShell title="종합부동산세 계산 결과" subtitle="정밀 계산 필요" stepIdx={total} stepTotal={total} onBack={() => setReport(null)} tag="LIVE">
            <JTFallbackBlocked gaps={compGaps} onRetry={runAnalysis} reason={compFallbackGaps(answers, { precise: true }).length > 0 ? 'input' : 'engine'} />
            ...
            <div className="jt-report-result__section" style={{ background: 'var(--bg-1,#f7f5f0)', borderLeft: '4px solid var(--accent,#2a6d4f)', padding: '14px 18px', marginBottom: 16 }}>
              <p style={{ margin: '0 0 12px', lineHeight: 1.65 }}>
                ...
                {hasCredit
                  ...
                  /* 「가까운 값」은 바로 아래 «미반영» 목록과 모순된다 — 재산세 공제·세부담 상한이 빠진
                     «상한 추정»이므로 실제 고지세액은 이보다 낮아진다 (260806 Codex P2) */
                  : '재산세 공제분과 세부담 상한을 빼지 않은 «최대치 추정»이라, 실제 고지세액은 이보다 낮아질 수 있어요(다주택은 세액공제가 없습니다). 부부 공동명의·합산배제 임대주택 등은 상담으로 확인하세요.'}
              </p>
