...
function crCalc(input) {
  ...
  const loss = crNum(input.otherLoss);          // 같은 해 다른 코인 손실
...
const CR_QS = [
  ...
  { id: 'salePrice', section: '매도 금액', q: '얼마에 파실 예정인가요? (원)', money: true, placeholder: '예: 50,000,000',
    sub: '1년 동안 판 금액을 모두 더해 넣으세요. 가상자산 세금은 건별이 아니라 «그 해 전체»를 합산해 계산합니다(소득세법 §84 3호).' },
  { id: 'acqPrice', section: '실제 취득가액', q: '실제로 얼마에 사셨나요? (원)', money: true, placeholder: '예: 10,000,000',
    sub: '처음 매수할 때 실제로 낸 금액입니다. 여러 번 나눠 샀다면 모두 더하세요. 채굴·에어드랍 등으로 받아 취득가액을 알기 어려운 경우는 별도 규정이 있습니다(아래 안내 참조).' },
  ...
  { id: 'otherLoss', section: '다른 코인 손실', q: '같은 해에 손해 보고 판 코인이 있나요? 손실액 합계는? (원)', money: true, placeholder: '예: 3,000,000',
    sub: '가상자산 소득은 그 해 전체를 합산해 계산하므로, 같은 해에 본 손실은 이익과 상계됩니다. 다만 «다음 해로 이월»해서 공제하는 규정은 현재 법에 없습니다. 없으면 0을 넣으세요.' },
];
      ...
      <div className="jt-container">
        ...
        <section className="jt-report-result__section">
          ...
          <div style={{ border: '1px solid #e5e1d9', borderRadius: 8, overflow: 'hidden' }}>
            {[
              ...
              ...(r.loss ? [{ k: 'Step 4. 같은 해 다른 코인 손실 통산', v: -r.loss, note: '연간 합산 기준 (§84 3호 «해당 과세기간의»)' }] : []),
              { k: '가상자산소득금액', v: r.income, note: '양도가액 − 필요경비' + (r.loss ? ' − 손실' : '') },
        ...
        <section className="jt-report-result__section" style={{ background: '#f7f5f0', padding: '15px 18px', borderRadius: 8 }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 19, fontSize: 14, lineHeight: 1.85 }}>
            ...
            <li><strong>같은 해 손실은 상계되지만, 다음 해로 이월하는 규정은 없습니다.</strong> 손익이 큰 해에는 실현 시점 관리가 중요합니다.</li>
